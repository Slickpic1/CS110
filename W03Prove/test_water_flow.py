...
def test_water_column_height():
    #Check inputs to make sure they work as they should
    assert wf.water_column_height(0.0,0.0) == 0.0
    assert wf.water_column_height(0.0,10.0) == 7.5
    assert wf.water_column_height(25.0,0.0) == 25.0
    assert wf.water_column_height(48.3,12.8) == 57.9

def test_pressure_gain_from_water_height():
    #Check inputs to makre sure they work
...
def test_pressure_loss_from_pipe():
    #Check inputs to make sure no wrong values are returned
...
def test_reynolds_number():
    assert wf.reynolds_number(0.048692,0.00) == approx(0, abs = 1)
...
def test_kpa_to_psi():
    assert wf.kpa_to_psi(1) == approx(0.145038, abs = 0.001)
    assert wf.kpa_to_psi(1.9909) == approx(0.288755632, abs = 0.001)
